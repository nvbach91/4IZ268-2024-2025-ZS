$(document).ready(() => {
    ...
    const saveToHistory = (word) => {
        ...
        appendToHistory(trimmedWord, date)
        updateAutocomplete();
    };
    ...
    const appendToHistory = (word, date) => {
        const data = getFromHistory(libraryName);
        const historyListItem = $(`<button type="button" class="btn btn-light m-1 text-start d-flex justify-content-between align-middle">${word}<span class="date text-secondary">${date}</span></button>`);
