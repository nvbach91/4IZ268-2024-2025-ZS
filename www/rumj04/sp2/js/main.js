...
const getCompletionKey = (eventId) => `completed:${eventId}`;

const setTaskCompletion = (eventId, completed) => {
  localStorage.setItem(getCompletionKey(eventId), completed);
};

const isTaskCompleted = (eventId) => {
  return localStorage.getItem(getCompletionKey(eventId)) === 'true';
};
...
const createTaskElement = (
  ...
  // Check and set initial completion state
  if (isTaskCompleted(eventId)) {
    completionCircle.classList.add("completed");
    li.classList.add("completed");
  }

  completionCircle.addEventListener("click", () => {
    const isCompleted = completionCircle.classList.toggle("completed");
    li.classList.toggle("completed");
    // Store the completion state
    setTaskCompletion(eventId, isCompleted);
  });
