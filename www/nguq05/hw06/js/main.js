...
const shiftString = (str, shift) => {
    ...
    for (let i = 0; i < str.length; i++) {
        result += shiftChar(arrayOfChar[i], shift);
    }
    return result;
};
...
const resultElement = document.querySelector('#decipheredText');
formElement.addEventListener('submit', (event) => {
    event.preventDefault();
    const data = new FormData(formElement);
    const cipheredText = Object.fromEntries(data)['textArea'];
    const key = data.get('key-value');
    if (isNaN(key)) {
        alert("Key value is missing.");
    };

    decipheredText.textContent = caesarDecipher(cipheredText, key);
})
