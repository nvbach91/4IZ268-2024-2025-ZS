...
const shiftString = (str, shift) => {
    return Array.from(str).map(char => shiftChar(char, shift)).join('');
...
document.getElementById('decipher-form').addEventListener('submit', (event) => {
    event.preventDefault(); // Zamezíme výchozímu odeslání formuláře
    const cipherText = event.target.cipher.value; // Získáme zašifrovaný text
    const key = parseInt(event.target.key.value, 10); // Získáme klíč jako celé číslo
    const result = caesarDecipher(cipherText.toUpperCase(), key); // Dešifrujeme text
    document.getElementById('output-field').innerText = result; // Zobrazíme dešifrovaný text
});
