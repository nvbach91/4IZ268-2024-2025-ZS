...
document.getElementById('input').addEventListener('submit', function(event) {
    ...
    const cipherText = document.getElementById('sifra').value;
    const shiftNumber = parseInt(document.getElementById('decipher').value);

    const result = caesarDecipher(cipherText, shiftNumber);
    document.getElementById('results').textContent = `Deciphered Text: ${result}`;
});
