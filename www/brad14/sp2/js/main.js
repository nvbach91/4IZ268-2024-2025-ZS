(() => {
    ...
    const renderWeatherData = (data) => {
        ...
        // add event listeners to each daily box
        $('.daily-box').on('click', (e) => {
            const box = $(e.target).closest('.daily-box')[0];
            const index = box.dataset.index;
            const lat = box.dataset.lat;
            const lon = box.dataset.lon;
            renderDayDetail(Number.parseFloat(index), Number.parseFloat(lat), Number.parseFloat(lon));
        });
    }
    ...
    const renderDailyContent = (dailyData, lat, lon) => {
        ...
        dailyData.forEach((day, index) => {
            ...
            const dayHtml = `
            <div class="daily-box" data-index="${index}" data-lat="${lat}" data-lon="${lon}">
    ...
    const renderDayDetail = async (index, lat, lon, weatherData) => {
        ...
        // add event listeners to each daily box
        $('.hour-box').on('click', (e) => {
            const box = $(e.target).closest('.hour-box')[0];
            const index = box.dataset.index;
            const lat = box.dataset.lat;
            const lon = box.dataset.lon;
            selectDay(Number.parseFloat(index), Number.parseFloat(lat), Number.parseFloat(lon));
        });
    }
    ...
    const updateRecentSearches = () => {
        ...
        // add event listeners to each trash icon
        $('.fa-trash').on('click', (e) => {
            e.stopPropagation();
            const location = e.target.closest('#trash-icon').dataset.location;
            removeLocationFromRecentSearches(location);
        });

        // add event listeners to each location
        $('.location').on('click', (e) => {
            const location = e.target.closest('.location').dataset.location;
    ...
    const renderRecentSearchesContent = () => {
        ...
        locations.forEach((location) => {
            const locationHtml = `
                <div class="location" data-location="${location}">
                    ${location}
                    <span id="trash-icon" data-location="${location}">
     ...
     */
    const removeLocationFromRecentSearches = (location) => {
        const locations = getLocations();
        const index = locations.indexOf(location);
        if (index > -1) {
            locations.splice(index, 1);
     ...
     * 
     * This function sets a timeout to show a loading wrapper after 400 milliseconds.
     * Once the wrapper is shown, it adds a 'visible' class to the loader after an additional 200 milliseconds.
     * 
     * @function
     * @name addLoadingUi
     */
    ...
    /**
     * Removes the loading UI by clearing the loader timeout, 
     * hiding the loader element, and sliding up the loader wrapper.
     */
